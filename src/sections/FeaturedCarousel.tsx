import useEmblaCarousel from 'embla-carousel-react';
import { FEATURED_SOFTWARE, formatPrice } from '@/data';
import SectionHeader from '@/components/SectionHeader';
import {
  Crosshair,
  Eye,
  Wrench,
  Zap,
  ShieldOff,
  Trophy,
  Star,
  ChevronLeft,
  ChevronRight,
  Flame,
  type LucideIcon,
} from 'lucide-react';

const ICON_MAP: Record<string, LucideIcon> = {
  Crosshair,
  Eye,
  Wrench,
  Zap,
  ShieldOff,
  Trophy,
};

export default function FeaturedCarousel() {
  const [emblaRef, emblaApi] = useEmblaCarousel({
    align: 'start',
    dragFree: true,
    containScroll: 'trimSnaps',
  });

  const scrollTo = (selector: string) => {
    document.querySelector(selector)?.scrollIntoView({ behavior: 'smooth' });
  };

  return (
    <section id="software" className="py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-6">
        <SectionHeader
          eyebrow="FEATURED SOFTWARE"
          headline="Top-Rated Tools"
          subtitle="Undetected, auto-updated, and ready to download the second you subscribe."
        />

        <div className="relative mt-14">
          <div className="overflow-hidden" ref={emblaRef}>
            <div className="flex gap-5">
              {FEATURED_SOFTWARE.map((software) => {
                const Icon = ICON_MAP[software.icon] || Crosshair;
                return (
                  <div
                    key={software.id}
                    className="group min-w-0 flex-[0_0_85%] sm:flex-[0_0_46%] lg:flex-[0_0_31%]"
                  >
                    <div className="flex h-full flex-col rounded-xl border border-best-border bg-best-elevated p-6 transition-all duration-300 hover:-translate-y-1 hover:border-best-cyan/60 hover:shadow-cyan-glow">
                      {/* Top row */}
                      <div className="flex items-start justify-between">
                        <div
                          className="flex h-12 w-12 items-center justify-center rounded-lg"
                          style={{ background: `${software.accent}1A`, color: software.accent }}
                        >
                          <Icon className="h-6 w-6" />
                        </div>
                        <span className="flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 font-heading text-[11px] font-semibold uppercase tracking-wider text-emerald-400">
                          <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />
                          {software.status}
                        </span>
                      </div>

                      <h3 className="font-heading mt-5 text-xl font-bold text-white">
                        {software.name}
                      </h3>
                      <p className="mt-1 text-sm text-best-caption">{software.game}</p>

                      {/* Rating */}
                      <div className="mt-3 flex items-center gap-2">
                        <div className="flex gap-0.5">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <Star
                              key={s}
                              className={`h-3.5 w-3.5 ${
                                s <= Math.round(software.rating)
                                  ? 'fill-best-gold text-best-gold'
                                  : 'fill-best-border text-best-border'
                              }`}
                            />
                          ))}
                        </div>
                        <span className="text-xs text-best-muted">
                          {software.rating} ({software.reviews})
                        </span>
                      </div>

                      {/* Urgency */}
                      <p className="mt-3 flex items-center gap-1.5 text-xs text-best-gold">
                        <Flame className="h-3.5 w-3.5" />
                        {software.boughtToday} people bought this today
                      </p>

                      {/* Price + CTA */}
                      <div className="mt-auto flex items-center justify-between border-t border-best-border pt-5">
                        <div>
                          <span className="font-display text-lg font-bold text-white">
                            {formatPrice(software.price)}
                          </span>
                          <span className="ml-1 text-xs text-best-caption">{software.period}</span>
                        </div>
                        <button
                          onClick={() => scrollTo('#pricing')}
                          className="rounded-lg bg-best-cyan/10 px-4 py-2 font-heading text-xs font-bold uppercase tracking-widest text-best-cyan transition-all duration-200 hover:bg-best-cyan hover:text-best-bg hover:shadow-cyan-glow"
                        >
                          Subscribe
                        </button>
                      </div>
                    </div>
                  </div>
                );
              })}
            </div>
          </div>

          {/* Arrows */}
          <button
            onClick={() => emblaApi?.scrollPrev()}
            aria-label="Previous"
            className="absolute -left-4 top-1/2 z-10 hidden h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-best-border bg-best-elevated text-best-muted transition-all hover:border-best-cyan hover:text-best-cyan hover:shadow-cyan-glow md:flex"
          >
            <ChevronLeft className="h-5 w-5" />
          </button>
          <button
            onClick={() => emblaApi?.scrollNext()}
            aria-label="Next"
            className="absolute -right-4 top-1/2 z-10 hidden h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-best-border bg-best-elevated text-best-muted transition-all hover:border-best-cyan hover:text-best-cyan hover:shadow-cyan-glow md:flex"
          >
            <ChevronRight className="h-5 w-5" />
          </button>
        </div>
      </div>
    </section>
  );
}
